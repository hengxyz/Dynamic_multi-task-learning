...
def main(args):
    ...
    if args.evaluate_express:
        ...
        # loss_verif_percentage_max = tf.minimum(loss_verif_percentage_softmax, 0.9999)
        # loss_verif_percentage = tf.maximum(loss_verif_percentage_max, 0.0001)
        ...
        train_op_weights, grads_weights, grads_clip_weights = train_BP.train(loss_for_weights, global_step,
                                                                              args.optimizer,
            ...
            while epoch < args.max_nrof_epochs:
                ...
                learning_rate_ \
                    = train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op,
                            ...
                            train_op_weights, grads_weights, grads_clip_weights, loss_for_weights, loss_full,
                            grads_full, grads_clip_full, logits_lossweights_embedings, learning_rate_dyn, learning_rate_dyn_placeholder,
                            logits_lossweights)
...
def train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder,
          ...
          grads_weights, grads_clip_weights, loss_for_weights, loss_full, grads_full, grads_clip_full, logits_lossweights_embedings,
          learning_rate_dyn, learning_rate_dyn_placeholder, logits_lossweights):
    batch_number = 0
    ...
    if args.learning_rate_dyn>0.0:
        ...
        lr_dyn = facenet.get_learning_rate_from_file(learning_rate_schedule_file, epoch_current)
    ...
    while batch_number < args.epoch_size:
        ...
        if (batch_number % 100 == 0):
            ...
            loss_expr_percentage_, loss_for_weights_, loss_full_, grads_full_, grads_clip_full_, _, logits_lossweights_embedings_, \
            learning_rate_dyn_, grads_weights_, grads_clip_weights_, logits_lossweights_, summary_str \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            ...
                            grads_clip_full, train_op_weights, logits_lossweights_embedings, learning_rate_dyn,
                            grads_weights, grads_clip_weights, logits_lossweights, summary_op], feed_dict=feed_dict)
            ...
            softmax_acc_expr_, cross_entropy_verif_, diff_mean_, centers_, _, loss_verif_percentage_, \
            loss_expr_percentage_, loss_for_weights_, loss_full_, grads_full_, grads_clip_full_, _, logits_lossweights_embedings_, \
            learning_rate_dyn_, grads_weights_, grads_clip_weights_, logits_lossweights_\
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            prelogits_center_loss_verif_n, cross_entropy_mean_verif, cross_entropy_mean_expr,
                            centers_cts_batch_reshape, logits_id, logits_expr, label_batch_id, label_batch_expr,
                            image_batch, learning_rate, softmax_acc_verif, softmax_acc_expr,
                            ...
                            loss_expr_percentage, loss_for_weights, loss_full,grads_full,
                            grads_clip_full, train_op_weights, logits_lossweights_embedings,learning_rate_dyn,
                            grads_weights, grads_clip_weights, logits_lossweights], feed_dict=feed_dict)
        print("step %d"%step)
        ...
        grad_clip_sum = 0
        # grads = grads_full_
        # grads_clip = grads_clip_full_
        grads = grads_weights_
        grads_clip = grads_clip_weights_
        ...
        print('\n')
        for i, grad_clip in enumerate(grads_clip):
            grad_clip_norm = LA.norm(np.asarray(grad_clip))
            grad_clip_sum += grad_clip_norm
            print ('grad_clip_%dth: %f  '%(i,grad_clip_norm), end='')
        print('\n')
        print('>>>>>>>>>>>>>>>>>>>>>>>>>>> Gradient norm is: %f' % grads_sum)
        print('>>>>>>>>>>>>>>>>>>>>>>>>>>> Gradient clip norm is: %f' % grad_clip_sum)
        ...
        print('Epoch: [%d][%d][%d/%d]\tTime %.3f\n \
            ...
            \tloss_verif_percentage: %2.4f\tloss_expr_percentage: %2.4f\tloss_dynamic_weight: %2.4f\texp(logits_lossweights_emb): %f %f\texp(logits_lossweights): %f %f \tloss_full: %2.4f \n \
            ...
            (epoch, epoch_current, batch_number + 1, args.epoch_size, duration, loss_verif_, loss_expr_, cross_entropy_mean_verif_, \
             ...
             np.exp(np.mean(logits_lossweights_embedings_[:,0])), np.exp(np.mean(logits_lossweights_embedings_[:,1])), \
             np.exp(np.mean(logits_lossweights_[:,0])), np.exp(np.mean(logits_lossweights_[:,1])), loss_full_, softmax_acc_verif_, \
        ...
        with open(os.path.join(log_dir, 'grads.txt'), 'at') as f:
            f.write('%f\t%f\n' % (grads_sum, grad_clip_sum))
        with open(os.path.join(log_dir, 'logits_weight_layer.txt'), 'at') as f:
            f.write('%f %f %f %f\n' % (np.exp(np.mean(logits_lossweights_embedings_[:,0])), \
                                  np.exp(np.mean(logits_lossweights_embedings_[:,1])), \
                                  np.exp(np.mean(logits_lossweights_[:,0])), \
                                  np.exp(np.mean(logits_lossweights_[:,1])),))

...
def inference_distinguer(net, keep_probability, phase_train=True, weight_decay=0.0, reuse=None):
    batch_norm_params = {
        # Decay for the moving averages.
        'decay': 0.995,
        # epsilon to prevent 0s in variance.
    ...
    with slim.arg_scope([slim.conv2d],
        ...
        # return inception_resnet_v1_expression(images, is_training=phase_train,
        #                                            dropout_keep_prob=keep_probability, reuse=reuse)
        return inception_resnet_distinguer(net, is_training=phase_train,
                                                   dropout_keep_prob=keep_probability, reuse=reuse)


def inception_resnet_distinguer(net, is_training=True,
                                        dropout_keep_prob=1.0,
                                        reuse=None,
                                        scope='InceptionResnetV1_distinguer'):
    """Creates the Inception Resnet V1 model.
    Args:
      inputs: a 4-D tensor of size [batch_size, height, width, 3].
      num_classes: number of predicted classes.
    ...
    with tf.variable_scope(scope, 'InceptionResnetV1_distinguer', [net], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
                            is_training=is_training):
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
                                stride=1, padding='SAME'):
